...
- `python3 vpngate.py` regenerates media stack config on every normal run.
- `docker compose up -d` runs for the media stack on every normal run.
...
- Decide whether VPN Gate config generation belongs in normal convergence or an explicit media-stack operation.
